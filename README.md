...
| LVS: `No matching ".SUBCKT" statement for "X"` | The **source netlist is incomplete** — subcircuit `X` is instantiated but never defined. Usually a subcell has **no schematic/auCdl view** or is excluded by `simStopList`, so `si` didn't emit its `.SUBCKT`. The failure's **DEBUG HELP** block prints a `diff` of the generated netlist vs a known-good one and a `comm` that lists exactly which subckts are missing. Fix: give the subcell a schematic, or reuse the known-good netlist (prefill its `_calibre.lvs_` and keep the same cell so `SOURCE PATH` is reused). |
