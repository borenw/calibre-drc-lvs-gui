...
| LVS: `No matching ".SUBCKT" statement for "X"` | The **source netlist is missing a definition** for subckt `X`. Most often the CDL needs a device/std-cell `.INCLUDE` that auCdl doesn't emit — the tool now **injects** it: set `netlist_include` (Config) to that CDL path, or it auto-mirrors any `.INCLUDE` from a known-good reference netlist (a prefilled `_calibre.lvs_`'s `SOURCE`). If instead a *subcell* has **no schematic/auCdl view** (or is on `simStopList`), give it a schematic. The failure's **DEBUG HELP** block prints a `diff` of the generated vs known-good netlist and a `comm` listing exactly which subckts/includes are missing. |
